<template>
    <div v-if="loaded">
        <section class="section" :class='templateVariables.object.settings.css_class + " is-width-" + templateVariables.object.settings.width + " has-gutter-enabled"'>
...
<script>
import ShopifyBuyClient from '../../services/shopify-buy-client';
...
export default {
    ...
    methods: {
        ...
        async fetchCollction(collection) {
            ...
            const collection_id = btoa(collection_name);
            let result = await ShopifyBuyClient.collection.fetchWithProducts(collection_id, {productsFirst: 20});
